...
# ═══ КАРТОЧКА = 100lvh + ЧАСЫ, И НИКАКИХ УГАДАННЫХ ЧИСЕЛ (15.09.2026, 00:30) ═
# Два чистых замера владельца на одной сборке дали недостающую линейку:
#   адрес СВЕРХУ: lvh 956 (= экран), окно 742, забрано 214
#   адрес СНИЗУ:  lvh 836 (= экран − 120), окно 796, забрано 160
# `100lvh` — это ВСЯ высота, доступная странице в текущем режиме: при адресе снизу
# Safari держит 120 точек под нижнюю панель даже в «большом» вьюпорте, при адресе
# сверху отдаёт экран целиком. Раньше мы пытались вывести это из `screen.height`,
# который про режим ничего не знает, — отсюда и весь подбор.
#
# Формула теперь сходится сама:
#   высота карточки = 100lvh + часы;  подъём = часы.
# Адрес снизу: 836 + 62 = 898, стоит от −62 до 836 — низ ровно по границе доступного.
# Адрес сверху: 956 + 62 = 1018, от −62 до 956 — низ ровно по низу экрана.
#
# И проверка вменяемости: если `lvh` не похож на правду (больше экрана или меньше
# двух третей его), режим не включаем вовсе — уходим в фолбэк, где карточка равна
# окну. Лучше ровная лента без захода под панели, чем красивая теория на числе,
# которого мы не понимаем.
ВМЕНЯЕМ = ("((window.__lvh||0)>screen.height*0.66 && (window.__lvh||0)<=screen.height)")
УСЛ_OLD = "      && (screen.height - ОКНОБЕЗПАНЕЛЕЙ()) <= 175;"
assert html.count(УСЛ_OLD) == 1, 'условие режима'
html = html.replace(УСЛ_OLD,
    "      && (screen.height - ОКНОБЕЗПАНЕЛЕЙ()) <= 175\n"
    "      && " + ВМЕНЯЕМ + ";   /* опыт: lvh должен быть похож на правду */")

ВЫС_OLD = """      корень.style.setProperty('--xscr', screen.height+'px');"""
assert html.count(ВЫС_OLD) == 1, 'высота карточки'
html = html.replace(ВЫС_OLD,
    """      корень.style.setProperty('--xscr', (ОКНОБЕЗПАНЕЛЕЙ()+(ЧАСЫ[screen.height]||0))+'px');""")

ПОДЪЁМ_OLD = """        const T=Math.max(0, Math.round(screen.height-окно-часы));"""
assert html.count(ПОДЪЁМ_OLD) == 1, 'формула подъёма'
html = html.replace(ПОДЪЁМ_OLD, """        const T=часы;   /* карточка уже ростом lvh+часы: поднять её надо ровно на часы */""")
