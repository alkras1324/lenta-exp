...
# ═══ ОПЫТ 14.09.2026: ЭКРАН МЕРИМ САМИ — `screen` ЗАВИСИТ ОТ НАСТРОЙКИ ══════
# ПРИЧИНА НАЙДЕНА ВЛАДЕЛЬЦЕМ, А НЕ ВЫВЕДЕНА: «Настройки → Экран и яркость → Вид»
# в положении «Увеличено» (Display Zoom). Он переключил его у себя и ВОСПРОИЗВЁЛ
# чужую беду — снизу появилась чёрная полоса. Прежняя догадка «дело в версии Safari
# 26.6.1 против 26.6» была построена на одном наблюдении и снята.
#
# Что делает эта настройка: iOS рисует в МЕНЬШЕМ логическом разрешении и растягивает.
# У одного и того же телефона `screen` становится 414x896 вместо 440x956, а окно с
# иконки — 894 вместо 956. Наша таблица высот часов индексировалась по `screen.height`:
# 896 в ней есть, и часы брались 44 вместо 62 — карточка уезжала под часы на 18 точек
# меньше, чем надо. Молча, потому что «похожий» экран в таблице нашёлся.
#
# Опора — измеренный `100lvh`: 956 / 956 / 812 на всех замерах, при любом положении
# переключателя, и во вкладке, и с иконки. Переводим на него И таблицу часов, И условие
# включения стеклянного режима. Порог «забрано <= 175» остаётся по `screen`: он отличает
