...
(function(){
  ...
  try{
    ...
    st.textContent=
      ...
      /* у ролика коробки нет: ролик и фото под ним — во всю карточку, одним размером */
      '.photo.boxy:has(video.fg) .fg,.photo.boxy:has(video.fg) .still{object-fit:cover!important;transform:none!important}'+
      '.photo.boxy:has(video.fg) .bg{display:none!important}'+
